        ...
        try {
            mysqlbHLS_service_1.default.query("SELECT platform FROM table WHERE uniqid=?", [id]).then((result) => __awaiter(this, void 0, void 0, function* () {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    if (stats.isFile()) {
                        let s3Path = "videos/";
                        const normalizedFilePath = filePath.split(path_1.default.sep).join(path_1.default.posix.sep);
                        if (normalizedFilePath.includes(".ts") || normalizedFilePath.includes(".m3u8")) {
                            const parts = normalizedFilePath.split(path_1.default.posix.sep);
                            const index = parts.indexOf("converted");
                            s3Path += `${parts[index + 1]}/${parts[index + 2]}`;
                            (0, converter_service_1.updateStatus)('Uploading streaming file to S3', id);
                        }
                        if (normalizedFilePath.includes("download")) {
                            const parts = normalizedFilePath.split(path_1.default.posix.sep);
                            const index = parts.indexOf("converted");
                            s3Path += `${parts[index + 1]}/${parts[index + 2]}/${parts[index + 3]}`;
                            (0, converter_service_1.updateStatus)('Uploading download file to S3', id);
                        }
                        yield uploadFileToS3(filePath, bucketName, s3Path, id);
                    }
                }
            }));
            console.log("Uploaded to S3");
            (0, converter_service_1.updateStatus)('uploaded to S3', id);
        }
        catch (err) {
            mysqlbHLS_service_1.default.query("SELECT platform FROM table WHERE uniqid=?", [id]).then((result) => {
                mysqldb_service_1.default
